...
void BallFollower::decideBallPositin(double x_angle, double y_angle)
{
  ...
  if (ball_x_angle > 0) //aslinya 0   
    approach_ball_position_ = OnLeft;

  else{ //(ball_x_angle < -5) //aslinya else doang
    approach_ball_position_ = OnRight;

  // else if (ball_x_angle <= 5 && ball_x_angle > 0)
    // {setWalkingParam(3, 0.0, 0.0);
    // approach_ball_position_ = OnLeft;};

  // else //(ball_x_angle >= -10 && ball_x_angle < 0)
    // {setWalkingParam(-3, 0.0, 0.0);
    // approach_ball_position_ = OnRight;};
}
